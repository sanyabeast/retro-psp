...
function Character:on_tick(delta)
    ...
    self.transform.position[1] = self.transform.position[1] +
                                     (self.direction[1] *
                                         (delta * self.walking_speed))
    self.transform.position[2] = self.transform.position[2] +
                                     (self.direction[2] *
                                         (delta * self.walking_speed))
    ...
    if (title_comp ~= nil) then title_comp:update({text = self.uuid}) end
end
return Character
