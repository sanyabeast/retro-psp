...
function GameObject:start(delta)
    ...
    for i, child in pairs(self.children) do child:start() end
    for i, component in pairs(self.components) do
        if (component.enabled) then component:start() end
    end
end
function GameObject:add_component(comp_data)
    -- log("GameObject", "adding comp " .. comp_data.name)
    local new_comp = Assets:create_component(comp_data.name,
                                             merge(comp_data.params, {
        game_object = self,
...
-- TRANSFORMATIONS
function GameObject:translate(x, y, z) self.transform:translate(x, y, z) end
return GameObject
